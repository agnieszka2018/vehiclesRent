...
BOOST_AUTO_TEST_SUITE()
    ...
    BOOST_AUTO_TEST_CASE(TestRentDuration) {
        ...
        Rent *testRent = new Rent(ldt, testAuto, testClient);

        BOOST_REQUIRE_EQUAL(testRent->rentDuration(), 0);

        testRent->returnVehicle(); //zwracam pojazd

        BOOST_REQUIRE_GT(testRent->rentDuration(), 0);

        BOOST_REQUIRE_EQUAL(testRent, testClient->getActuallRent()); //test czy klient ma wypożyczenie?
    }
